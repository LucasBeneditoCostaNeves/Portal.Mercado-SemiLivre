      ...
      {departments.map((dept) => (
        ...
        >
          <div className="w-10 h-10 bg-[var(--color-brand)] rounded-full flex items-center justify-center">
            <i className={`ti ${dept.icon} text-xl text-[var(--color-brand-dark)]`} aria-hidden="true" />
          </div>
          <span className="text-xs text-[var(--color-text-secondary)] text-center leading-tight">{dept.label}</span>
        </button>
      ))}
    </div>
  );
