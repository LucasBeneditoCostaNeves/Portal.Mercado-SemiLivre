...
O sistema SHALL renderizar a página de detalhe de produto na rota `/(home)/produto/[id]` como Server Component, herdando o layout (Navbar + SubNav) do route group `(home)`.

#### Scenario: Acesso com ID válido renderiza a página completa
- **WHEN** o usuário acessa `/produto/:id` com um ID de produto existente e ativo
- **THEN** a página é renderizada com todos os blocos: breadcrumbs, galeria, área de info do produto, price box, info cards e tabela de especificações
...
#### Scenario: Loading state exibe skeleton durante o fetch
- **WHEN** o fetch da API ainda não completou
