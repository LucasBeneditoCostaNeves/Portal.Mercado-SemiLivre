...
Especificação do painel de filtros da página de resultados de pesquisa, incluindo layout responsivo (sidebar/drawer), controles de ordenação, filtros de frete grátis, faixa de preço, avaliação mínima, marca e categoria, além de ações de aplicar e limpar filtros.

## Requirements

### Requirement: Painel de filtros com layout responsivo
A página de resultados SHALL renderizar um `FiltersPanel` que aparece como sidebar fixa à esquerda do grid em desktop (≥1024px) e como drawer deslizante de baixo para cima acionado por botão em mobile (<1024px). O estado dos filtros SHALL ser sincronizado com a URL via query params.

#### Scenario: Sidebar visível em desktop
- **WHEN** a viewport tem 1024px ou mais
- **THEN** o `FiltersPanel` é exibido como sidebar fixa à esquerda do grid, sem necessidade de clicar em botão
...
O botão "Filtros" no mobile SHALL exibir um badge numérico com a quantidade de filtros ativos (excluindo `q` e `sort`/`order` da contagem).

#### Scenario: Sem filtros ativos
- **WHEN** nenhum filtro além da busca está ativo
...
- **WHEN** usuário compartilha a URL com `?sort=rating&order=desc`
- **THEN** ao acessar a URL, a opção "Melhor avaliação" aparece pré-selecionada
...
- **THEN** `freeShipping` é removido da URL e todos os produtos são exibidos
...
#### Scenario: Apenas máximo definido
- **WHEN** o usuário define apenas o preço máximo
- **THEN** a URL inclui apenas `maxPrice={n}` sem `minPrice`
...
O painel SHALL exibir uma lista de marcas com checkboxes, derivadas dos resultados atuais. A seleção de uma ou mais marcas adiciona `brand={marca}` (múltiplos valores separados por vírgula) à URL.

#### Scenario: Selecionar uma marca
- **WHEN** o usuário seleciona a marca "Samsung"
- **THEN** a URL inclui `brand=Samsung` e apenas produtos dessa marca são exibidos
...
### Requirement: Filtro de categoria por checkbox
O painel SHALL exibir uma lista de categorias com checkboxes, derivadas dos resultados atuais. A seleção de uma ou mais categorias adiciona `category={categoria}` à URL.

#### Scenario: Selecionar uma categoria
- **WHEN** o usuário seleciona a categoria "Celulares"
- **THEN** a URL inclui `category=Celulares` e apenas produtos dessa categoria são exibidos

---

### Requirement: Botões Aplicar e Limpar filtros
O painel SHALL ter um botão "Aplicar filtros" que persiste as seleções atuais na URL e um botão "Limpar filtros" que remove todos os params de filtro da URL (mantendo apenas `q`).
...
- **THEN** todos os params de filtro são removidos da URL, mantendo apenas `?q={termo}`

#### Scenario: Filtros persistem na URL
- **WHEN** o usuário compartilha a URL com filtros ativos
