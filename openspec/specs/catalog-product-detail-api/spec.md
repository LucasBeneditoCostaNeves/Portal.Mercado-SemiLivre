...
Especificação do endpoint público de detalhe de produto do catálogo (`GET /catalog/products/:id`), com campos agregados no shape `CatalogProductDetail` e extensão do repositório de catálogo para suportar a busca por ID com eager loading das relações.
