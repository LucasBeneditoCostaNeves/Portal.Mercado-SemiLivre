...
- [x] 2.5 Testar os três endpoints manualmente com token válido e inválido
...
- [x] 4.1 Chamar `recordSearch(q)` no Server Component da página `/buscar` após validação do param `q` (somente se usuário autenticado)
...
- [x] 7.4 Verificar que usuário não autenticado não gera nenhum registro e não vê dropdown
- [x] 7.5 Verificar que falha no backend de histórico não quebra nenhuma ação do usuário
